...
def unet_no_dropout(pretrained_weights=None, input_size=(512, 512, 1), act = 'elu'):
    ...
    #conv4 = tf.keras.layers.Activation(swish)(conv4)
    # drop4 = tf.keras.layers.Dropout(0.5)(conv4)
    pool4 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(conv4)

    conv5 = tf.keras.layers.Conv2D(1024, 3, activation=act, padding='same', kernel_initializer='he_normal')(
        pool4)
    conv5 = tf.keras.layers.Conv2D(1024, 3, activation=act, padding='same', kernel_initializer='he_normal')(
        conv5)
    # drop5 = tf.keras.layers.Dropout(0.5)(conv5)
    ...
    up6 = tf.keras.layers.Activation(swish)(up6)
    merge6 = tf.keras.layers.concatenate([conv4, up6], axis=3)
    # merge6 = tf.keras.layers.concatenate([conv4, up6], axis=3)
    conv6 = tf.keras.layers.Conv2D(512, 3, activation=act, padding='same', kernel_initializer='he_normal')(
        merge6)
    ...
    up7 = tf.keras.layers.Conv2D(256, 2, activation=act, padding='same', kernel_initializer='he_normal')(
        tf.keras.layers.UpSampling2D(size=(2, 2))(conv6))
    merge7 = tf.keras.layers.concatenate([conv3, up7], axis=3)
    conv7 = tf.keras.layers.Conv2D(256, 3, activation=act, padding='same', kernel_initializer='he_normal')(
        merge7)
    ...
    up8 = tf.keras.layers.Conv2D(128, 2, activation=act, padding='same', kernel_initializer='he_normal')(
        tf.keras.layers.UpSampling2D(size=(2, 2))(conv7))
    merge8 = tf.keras.layers.concatenate([conv2, up8], axis=3)
    conv8 = tf.keras.layers.Conv2D(128, 3, activation=act, padding='same', kernel_initializer='he_normal')(
        merge8)
    ...
    # model.compile(optimizer = Adam(lr = 1e-4), loss = 'binary_crossentropy', metrics = ['accuracy'])
    model.compile(optimizer=tf.keras.optimizers.Adam(lr_scheduler), loss=combo_loss(alpha=0, beta=0.4),
...
lr_scheduler = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=5e-7,decay_steps=2720,decay_rate=0.9)
